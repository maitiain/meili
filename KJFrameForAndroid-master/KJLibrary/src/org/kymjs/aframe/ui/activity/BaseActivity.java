...
public abstract class BaseActivity extends KJFrameActivity {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
    	KJLoger.debug("KJActivityManager.create().getCount():"+KJActivityManager.create().getCount());
    	KJLoger.debug("mOpenBackListener:"+mOpenBackListener);
        if (mOpenBackListener && keyCode == KeyEvent.KEYCODE_BACK
                && KJActivityManager.create().getCount() < 2) {
        	KJLoger.debug("ViewInject getExitDialog....");
